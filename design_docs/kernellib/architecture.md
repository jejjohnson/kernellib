...
No numpyro. No scikit-learn in the core, and no numpy beyond what JAX
brings. The opt-in `kernellib.sklearn` adapters (extra `kernellib[sklearn]`)
are the only place scikit-learn is imported; see the decisions log.
...
| 2026-09-25 | Phase 1 kernel contract as built: `spectral_density` / `sample_frequencies` are not yet on `AbstractStationaryKernel`; they arrive with the spectral phase so no kernel is forced to stub them now |
| 2026-09-25 | `AbstractKernel` carries a non-abstract `pairwise` (raises `TypeError`) and an `is_pointwise` property (False). Composites are pointwise exactly when all children are, which is what `to_operator(implicit=True)` checks. `__call__` stays the only abstract method, so the `pyrox_gp.Kernel` alias is unaffected |
...
| 2026-09-25 | Spectral contract as built: `spectral_density(omega)` takes frequency *vectors* `(..., D)` (ARD needs them; the Laplace-eigenfunction path evaluates per-axis frequencies) and `sample_frequencies(key, n, d)` returns lengthscale-scaled draws. Neither is abstract. A kernel opts in through `unit_spectral_density(omega_sq, d)` and `sample_unit_frequencies(key, shape, dtype)` for its unit-variance, unit-lengthscale form, and the base class applies `det(Λ)`, `Λω` and the variance, so a user stationary kernel that only defines `shape` still works and raises `NotImplementedError` naming itself on the spectral path |
| 2026-09-25 | Matern frequencies are drawn from the joint multivariate Student-t (`g * sqrt(nu / u)`, one Gamma draw per frequency vector). pyrox-gp's coordinate-wise Student-t draw is the Matern spectrum only in 1-D; a test pins that it fails the characteristic-function check in 3-D. `RationalQuadratic` gets a sampler (Gamma scale mixture of Gaussians) but no density, which needs a Bessel K that JAX lacks |
| 2026-09-25 | `draw_rff_cosine_basis` / `evaluate_rff_cosine_paths` moved to `_spectral/_rff.py` with the pyrox-gp return layout: frequencies at unit lengthscale, lengthscale applied at evaluation, so pyrox-gp's pathwise samplers can pair one draw with hyperparameters cached on a conditioned GP. The kernellib version reads the hyperparameters straight off the kernel; the override arguments stay in the pyrox-gp wrapper. kernellib's pin rises to `gaussx>=0.2.0` in the same PR |
| 2026-09-25 | Feature maps as built: `fit(kernel, X)` takes the inputs as well as the kernel (the API sketch showed `fit(k)`), because an isotropic kernel does not know its input dimension and Nyström selects landmarks from `X`. The random maps store their draw at unit lengthscale and unit variance and read the kernel's hyperparameters at call time, so gradients reach them through a fitted map; Nyström forms `K_ZZ` at call time for the same reason. `operator(X)` is one shared zero-base `LowRankUpdate(Φ, Φ)` rather than per-map calls to `nystrom_operator` / `fastfood_operator`, which build the same operator |
| 2026-09-25 | Orthogonal random features take their directions from `geonnax.randfeat.orthogonal_blocks` but their lengths from the kernel's own radial law (norms of `sample_unit_frequencies`), so ORF works for Matern and RationalQuadratic, not just RBF. `FastFoodFeatures` does the same for FastFood's row lengths `S` (risk 11): norms of unit samples in `d_padded` dimensions, which reduces to `fastfood_params`' χ law for RBF and is exact for the elliptical Matern / RQ laws because their marginals on the unpadded coordinates are the `D`-dimensional laws. A test pins that keeping the RBF `S` for Matern-½ is biased |
| 2026-09-25 | `LaplaceEigenfunctionFeatures` takes half-widths `L` (scalar or per axis, the geonnax and pyrox-nn convention) rather than the sketched `bounds=(-L, L)`; with `L=None` they are `boundary_factor * max|X_d|` (default 1.5), with the Riutort-Mayol rule for choosing the factor in the docstring. The spectral density is evaluated at the per-axis frequency vectors, not at `sqrt` of the summed eigenvalue, which is what makes ARD exact; for an isotropic kernel the two agree (tested). `fit` evaluates the density once so a kernel without one fails there, not at the first call |
| 2026-09-26 | Heuristics as built: `estimate_lengthscale(X, method, *, percent, subsample, key, scale, ard)` with pysim's median / mean / Silverman / Scott rules and three fixes: Silverman and Scott are multiplied by the data's standard deviation (pysim returned the bare `n`-dependent factor), the `percent` neighbour index is counted from the subsample actually used, and self-distances are excluded. `ard=True` returns one lengthscale per dimension. The conversions are named for lengthscales (`lengthscale_to_gamma`, `gamma_to_lengthscale`) and the grid is `lengthscale_grid`; pysim's `get_gamma_grid`, which returned lengthscales, is not carried over |
| 2026-09-26 | Estimator contract as built: `AbstractEstimator` with `fit(X, y, *, key=None)` returning a fitted copy (`dataclasses.replace`), `predict`, `is_fitted` and a differentiable `loss` (MSE). Weights are always `alpha`; targets `(N,)` or `(N, C)`. `KRR(kernel, regularization, solver=gx.DenseSolver(), implicit=False)` solves `(K + λ n I) α = y`: the ridge is scaled by `n`, the Falkon convention, so the two agree when Falkon uses every point as a centre and `λ` does not need retuning as `N` grows. Any gaussx solver strategy works; `implicit=True` builds the `ImplicitKernelOperator` (concrete `λ` only, as with `to_operator`) |
| 2026-09-26 | Dependence as built: `hsic`, `cka`, `kernel_alignment` take `(kernel_x, kernel_y, X, Y)`; MMD is `mmd_squared(kernel, X, Y)` at the top level, matching the `functional` name (the sketch said `mmd`), with biased, unbiased and linear-time estimators. The randomised path computes the statistics from the feature matrices (`‖Φ̄xᵀΦ̄y‖²/n²` and the low-rank form of the unbiased estimator) rather than handing `LowRankUpdate` operands to `functional.hsic`, whose `center_kernel` materialises `N x N`. The `approx` map is a template fitted per kernel with split keys (once on the pooled sample for MMD). `permutation_test(statistic, X, Y, *, key, n_permutations, kind)` takes a `(X, Y) -> scalar` callable and a `kind` (`"independence"` shuffles `Y`, `"two_sample"` shuffles the pooled sample) instead of the sketch's `permutation_test(kl.hsic, kx, ky, X, Y)`, because the two nulls need different permutations. `hsic_input_gradient` is not a separate function: `jax.grad` of `hsic` with respect to the inputs is tested instead |
| 2026-09-26 | `Falkon(kernel, n_inducing, regularization, max_iter, tol, implicit=True, batch_size, jitter)` picks centres uniformly (key required when `M < N`), builds `falkon_preconditioner` on `kernel(Z, Z)` and runs `falkon_solve` against the streamed cross kernel; with every point a centre it equals `KRR` (tested). `EigenPro(kernel, epochs, batch_size, subsample_size, n_components, decay)` runs `lax.scan` SGD with `α_B -= (η/b) g` and `α_S += (η/(b m)) V D Vᵀ K_SB g`, the RKHS-normalised form of the EigenPro update; the spectral exponent is `decay` so that `alpha` is always the weights. Building it exposed three defects in the moved EigenPro primitives, fixed in a separate commit: the `n_components + 1`-step Lanczos misestimated the tail eigenvalue (now dense `eigh` of the already-dense `K_mm`), `β` subtracted `e_i(x)² / (m λ_i)` instead of `(1 - r_i) e_i(x)²`, and the step size used the unpreconditioned top eigenvalue |
| 2026-09-26 | scikit-learn compatibility through an opt-in adapter package, `kernellib.sklearn`, behind the `kernellib[sklearn]` extra. The core estimators stay immutable equinox modules (`fit` returns a fitted copy, so `jax.grad` goes through it); the adapters are mutable scikit-learn estimators that hold the fitted kernellib object (`model_`, `feature_map_`). `import kernellib` still loads no scikit-learn. Kernel fields are nested parameters (`kernel__lengthscale`, `kernel__kernels__0__variance`), set by rebuilding the kernel, so model selection searches over them; `kernel=None` means an RBF at the median-heuristic lengthscale. Regressors (`KernelRidge`, `FalkonRegressor`, `EigenProRegressor`) and transformers (all five feature maps) pass `check_estimator`; `HSIC` (with `normalize=True` for CKA) and `MMD` reimplement pysim's estimator-style classes with permutation p-values but are not predictors, so they follow the conventions without the checks |
